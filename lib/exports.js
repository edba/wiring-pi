...
// Array of callbacks by pin
var events  = require('events'),
    emitter = new events.EventEmitter();

/*
  All interrupts fire on the same context object
*/
c.interrupts.oninterrupt = function (pin, deltaTime) {
  emitter.emit(pin, deltaTime);
};

exports.wiringPiISR = function (pin, edge, callback) {
  emitter.on(pin, callback);
  // Do we need to track if a pin already has an interrupt registered?
  c.addInterruptListener(pin, edge);
}
